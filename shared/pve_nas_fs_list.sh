...
# {if ($5 ~ input_tran && $3 == 0 && ($4 != "LVM2_member" || $4 != "zfs_member") && $9 == "disk" && size >= stor_min && $10 == 0 && $13 !~ basic_disklabel && $14 == 0 && $15 == 0) { ssd_count++ }} END { if (ssd_count >= 1) print "Build a new LVM VG/LV - SSD Disks", "Select from "ssd_count"x SSD disks", $5, "-", "-", "TYPE05" } \
# # Type06: Build a new LVM VG/LV - HDD Disks
# {if ($5 ~ input_tran && $3 == 0 && ($4 != "LVM2_member" || $4 != "zfs_member") && $9 == "disk" && size >= stor_min && $10 == 1 && $13 !~ basic_disklabel && $14 == 0 && $15 == 0) { hdd_count++ }} END { if (hdd_count >= 1) print "Build a new LVM VG/LV - HDD Disks", "Select from "hdd_count"x HDD disks", $5, "-", "-", "TYPE06" }' \
# | sed '/^$/d' \
# | sort -t: -s -k 4,4 \
# | awk -F':' '!seen[$1$2]++')
...
lvm_option_labels=$(printf '%s\n' "$lvm_option_input" | cut -d: -f1,2,3,4)
lvm_option_values=$(printf '%s\n' "$lvm_option_input" | cut -d: -f5,6)
...
| awk -F':' '!seen[$1]++')
# Create selection labels & values
zfs_option_labels=$(printf '%s\n' "$zfs_option_input" | cut -d: -f1,2,3,4)
zfs_option_values=$(printf '%s\n' "$zfs_option_input" | cut -d: -f5,6)
...
# {if ($5 ~ input_tran && $3 == 0 && ($4 != "LVM2_member" || $4 != "zfs_member") && $9 == "disk" && size >= stor_min && $13 !~ basic_disklabel && $14 == 0 && $15 == 0) print "Basic single disk build", "Format "$1" to ext4", $5, $8} \
# # TYPE02: Mount existing NAS storage disk
# {if ($5 ~ input_tran && $3 != 0 && $4 == "ext4" && $9 == "part" && size >= stor_min && $13 ~ basic_disklabel && $14 == 0 && $15 == 0) print "Mount existing NAS storage disk", "Mount "$1" (disk label - "$13")", $5, $8} \
# # TYPE03: Destroy & wipe disk
# {if ($5 ~ input_tran && $3 != 0 && $4 == "ext4" && $9 == "part" && size >= stor_min && $13 !~ basic_disklabel && $14 == 0 && $15 == 0) print "Destroy & wipe disk", "Destroy disk /dev/"$3" (disk label - "$13")", $5, $8} \
# # TYPE04: Destroy, wipe and use partition
# {if ($5 ~ input_tran && $3 != 0 && ($4 == "ext2" || $4 == "ext3" || $4 == "ext4" || $4 == "btrfs" || $4 == "xfs") && $9 == "part" && size >= stor_min && $14 == 0 && $15 == 0) print "Destroy, wipe & use partition", "Use partition "$1" (disk label - "$13")", $5, $8}' \
# | sed '/^$/d' \
# | awk -F':' '!seen[$1$2$3$4]++')
...
# # TYPE04: Destroy, wipe and use partition
# {if ($5 ~ input_tran && $3 != 0 && ($4 == "ext2" || $4 == "ext3" || $4 == "ext4" || $4 == "btrfs" || $4 == "xfs") && $9 == "part" && size >= stor_min && $14 == 0 && $15 == 0) print "TYPE04", $0 }' \
# | sed '/^$/d' \
# | awk -F':' '!seen[$1$2$6$9]++')
...
| awk -F':' '!seen[$1$2$3$4]++')
# Create selection labels & values
basic_option_labels=$(printf '%s\n' "$basic_option_input" | cut -d: -f1,2,3,4)
basic_option_values=$(printf '%s\n' "$basic_option_input" | cut -d: -f5-)


#---- FS option count

# LVM option count
lvm_option_cnt=$(echo "$lvm_option_labels" | wc -l)
# ZFS option cnt
zfs_option_cnt=$(echo "$zfs_option_labels" | wc -l)
# Basic disk count
basic_option_cnt=$(echo "$basic_option_labels" | awk -F':' '$3 != "usb"' | wc -l)
# USB disk count
usb_option_cnt=$(echo "$basic_option_labels" | awk -F':' '$3 == "usb"' | wc -l)
