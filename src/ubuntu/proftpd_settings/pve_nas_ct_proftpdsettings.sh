...
if [[ $(ip r | head -n 1 | grep -n 'proto dhcp') ]]
then
  DHCP=1
else
  DHCP=0
...
echo
if [ -f "/etc/proftpd/conf.d/sftp.conf" ] || [ -f "/etc/proftpd/conf.d/global_default.conf" ] || [ -f "/etc/proftpd/conf.d/global_desktopdir.conf" ]
then
...
if [ "$PROFTPD_SETTING" = 0 ]
then
  msg "Modifying ProFTPd defaults and settings..."
  if [ "$(systemctl is-active proftpd)" = 'active' ]
  then
